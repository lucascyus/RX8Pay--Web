...
namespace agoraVai.pages
{
    ...
    {
        private const string connectionString = "Data Source=DESKTOP-E16QRJ4\\SQLEXPRESS01;Initial Catalog=SALARY_SYNC;Integrated Security=True";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                PreencherNomeUsuario(perfil);

                string codigoAleatorio = GerarCodigoAleatorio(5);
        ...
        protected void MeuBotao_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int idadeMinima = 18;
                    if (!VerificarIdade(dataNascimento, idadeMinima))
                    {
                ...
                if (!VerificarCpfVinculado(txtb_cpf.Text) && !VerificarRgVinculado(txtb_rg.Text) && !VerificarCtpsVinculado(txtb_ctps.Text) && !VerificarTituloVinculado(txtb_TituloEleitor.Text))
                {
                    InserirDados();
                }
                else
        ...
        private void InserirDados()
        {
            ...
            string nomeBanco = txtb_banco.Text;
            string conta = txtb_conta.Text;
            string numero = txtb_numero.Text;
            string agencia = txtb_agencia.Text;
            string tipo = txtb_tipo.Text;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command2 = new SqlCommand(query2, connection))
                {
                    ...
                    command2.Parameters.AddWithValue("@Nome", nomeBanco);
                    command2.Parameters.AddWithValue("@NumeroConta", conta);
                    command2.Parameters.AddWithValue("@NumeroBanco", numero);
                    command2.Parameters.AddWithValue("@AgenciaBancaria", agencia);
                    command2.Parameters.AddWithValue("@TipoConta", tipo);
        ...
        private bool VerificarIdade(DateTime dataNascimento, int idadeMinima)
        {
        ...
        private bool VerificarCpfVinculado(string cpf)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
        ...
        private bool VerificarRgVinculado(string rg)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
        ...
        private bool VerificarCtpsVinculado(string ctps)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
        ...
        private bool VerificarTituloVinculado(string tituloEleitor)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
